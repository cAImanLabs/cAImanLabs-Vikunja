...
func TestTaskCollection_ExpandSubtasksPaginatesRoots(t *testing.T) {
	...
	for i := 1; i <= 40; i++ {
		task := &Task{
			...
			CreatedByID: u.ID,
			Index:       int64(i),
			// Offset well past the fixture's kind_index range (all kind=task,
			// 1-51) so this test's own tasks - also kind=task by default -
			// don't collide with fixture rows on the global (kind, kind_index)
			// uniqueness constraint.
			KindIndex: int64(1000 + i),
		}
		_, err = s.Insert(task)
		require.NoError(t, err)
...
func setupSubtaskExpansionFixture(t *testing.T, u *user.User, title string, apply func(parent, sub *Task)) (project *Project, parent, sub *Task) {
	...
	// KindIndex offset well past the fixture's kind_index range (all
	// kind=task, 1-51) so these - also kind=task by default - don't collide
	// with fixture rows on the global (kind, kind_index) uniqueness constraint.
	parent = &Task{Title: "parent", ProjectID: project.ID, CreatedByID: u.ID, Index: 1, KindIndex: 1000}
	sub = &Task{Title: "sub", ProjectID: project.ID, CreatedByID: u.ID, Index: 2, KindIndex: 1001}
	if apply != nil {
		apply(parent, sub)
	}
