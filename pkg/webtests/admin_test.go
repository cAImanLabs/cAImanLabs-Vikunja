...
func TestAdmin_PatchTaskCompletedBy(t *testing.T) {
	e, err := setupTestEnv()
	require.NoError(t, err)
	license.SetForTests([]license.Feature{license.FeatureAdminPanel})
	defer license.ResetForTests()
	...
	t.Run("credits a different user", func(t *testing.T) {
		...
		res := adminReq(t, e, http.MethodPatch, "/api/v1/admin/tasks/2/completed-by", admin, `{"completed_by_id":2}`)
		assert.Equal(t, http.StatusOK, res.Code)
	...
	t.Run("rejects a task that isn't done", func(t *testing.T) {
		res := adminReq(t, e, http.MethodPatch, "/api/v1/admin/tasks/3/completed-by", admin, `{"completed_by_id":2}`)
		assert.Equal(t, http.StatusBadRequest, res.Code)
	})

	t.Run("nonexistent task returns 404", func(t *testing.T) {
		res := adminReq(t, e, http.MethodPatch, "/api/v1/admin/tasks/99999/completed-by", admin, `{"completed_by_id":2}`)
		assert.Equal(t, http.StatusNotFound, res.Code)
	})

	t.Run("nonexistent target user returns 404", func(t *testing.T) {
		res := adminReq(t, e, http.MethodPatch, "/api/v1/admin/tasks/2/completed-by", admin, `{"completed_by_id":99999}`)
		assert.Equal(t, http.StatusNotFound, res.Code)
	})
	...
	t.Run("non-admin caller gets 404", func(t *testing.T) {
		...
		res := adminReq(t, e, http.MethodPatch, "/api/v1/admin/tasks/2/completed-by", u, `{"completed_by_id":2}`)
		assert.Equal(t, http.StatusNotFound, res.Code)
	})
}

func TestAdmin_TaskCompletionStats(t *testing.T) {
	e, err := setupTestEnv()
	require.NoError(t, err)
	license.SetForTests([]license.Feature{license.FeatureAdminPanel})
	defer license.ResetForTests()
	...
	res := adminReq(t, e, http.MethodGet, "/api/v1/admin/tasks/completion-stats", admin, "")
	assert.Equal(t, http.StatusOK, res.Code)
	body := res.Body.String()
